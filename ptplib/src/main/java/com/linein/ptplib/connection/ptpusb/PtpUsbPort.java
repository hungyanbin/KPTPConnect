...
//	}
	...
	public short getDeviceStatus()
	{
		...
		PtpLog.d("getDeviceStatus: "+len);
		Packet p = new Packet(buffer);
		
		short status = p.getShortL(2);
		PtpLog.d("status is "+Packet.getHexString(status));
	...
	public void getExtendedEventData()
	{
		...
		PtpLog.d("getExtendedEventData: "+len);
		if(len>0)
		{
			Packet p = new Packet(buffer);
			short eventcode = p.getShortL(0);
			PtpLog.d("eventcode is "+Packet.getHexString(eventcode));
			PtpLog.d("transid is "+p.getIntL(2));
		}				
	}
