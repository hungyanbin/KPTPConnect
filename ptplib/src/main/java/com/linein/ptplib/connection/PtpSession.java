...
import com.linein.ptplib.datacallbacks.CreatePacketDataCallback;
...
public class PtpSession
{
    ...
    public void closeAndDisconnect()
    {
        ...
        try
        {
            if(hasOpenSession)
            {
                ...
                {
                    PtpLog.e("close session not ok: " + rne.getResponseCode());
                }
                hasOpenSession=false;
            }
        }
    ...
    public Packet getStorageId() throws ResponseNotOkException, ReceiveDataException, SendDataException
    {
        if(!ptp.isConnected())
            return null;
        if(!hasOpenSession)
            openSession();
        return requestPacket(PtpOc.GetStorageIDs, nextTransactionID(), null, USE_DEF_TIMEOUT);
    }
    ...
    public void getThumb(int handle, DataCallback cb) throws ReceiveDataException, ResponseNotOkException, SendDataException
    {
        if(!ptp.isConnected())
            return;
        if(!hasOpenSession)
        ...
        ptp.requestData(cb, PtpOc.GetThumb, nextTransactionID(), new int[]{handle}, USE_DEF_TIMEOUT);
    }
    ...
    public void getPartialObject(int handle, int offset, int size, DataCallback cb) throws ReceiveDataException, ResponseNotOkException, SendDataException
    {
        ...
        PtpLog.d("getPartialObject "+offset+" "+size);
    ...
    public List<Packet> canon_checkEvents() throws ResponseNotOkException, ReceiveDataException, SendDataException
    {
        ...
        Packet data = requestPacket(PtpOc.Canon_GetEvent, nextTransactionID(), null, USE_DEF_TIMEOUT);
